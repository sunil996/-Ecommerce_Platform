...
import { dbOrderItems } from "../model/dborders";
...
router.get("/showOrders",authenticateJWT,showOrders)
router.put("/orderStatus",authenticateJWT,manageSellerOrdersSchema,manageSellerOrders)
  ...
  let validSeller: any = {
    ...
    if (!result) {
     return res.send(functionsObj.output(500, "Account registration failed.", null));
      ...
      console.log(token);
     return res.send(functionsObj.output(201, "Account created",{result, token}));
    }
...
{
  const seller_id:number=req.seller_id;
  let resultOfOrders:any=await sellerObj.showSellerOrders(seller_id);
  
   if(!resultOfOrders[0]){
    return res.send(functionsObj.output(200,"sucess","no one have placed order of your items."));
    }
     return res.send(functionsObj.output(200,"sucess",resultOfOrders));  
}


function manageSellerOrdersSchema(req: any, res: any, next: any) {
...
async function manageSellerOrders(req: any, res: any) { 
  ...
  if(flag)
   {
     await objOrderItem.updateOrderStatus(id,"shipped")
    return  res.json({message:"order has accepted."})

   }else{
    await objOrderItem.updateOrderStatus(id,"rejected")
    return  res.json({message:"order has rejected."})
   }
}